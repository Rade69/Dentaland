...
- **Djelimičan izuzetak (Radovan, 30.8.2026):** email "Termin potvrđen" poruka (`send_appointment_confirmed`, NE `send_appointment_reminder` podsjetnik) SMIJE sadržati ime doktora — svjesno preokrenuta odluka, potvrđeno da je doktor manje osjetljiv podatak od tipa tretmana. Naziv usluge i dalje NIKAD ne ulazi ni u jednu poruku, ni u ovaj email. Podsjetnici (reminder, Telegram) ostaju pod punom minimizacijom (samo vrijeme), nepromijenjeno.
