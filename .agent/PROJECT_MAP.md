...
- `src/dentaland/timezone.py` — kanonska definicija `SARAJEVO` IANA zone
  (`Europe/Sarajevo`). Nakon REF-13 (26.8.2026) svih 15 ranije poznatih
  mjesta uvozi odavde (`main_window.py`, `day_view.py`, `week_view.py`,
  `blockout_panel.py`, `blockout_delete_confirm.py`,
  `schedule_controller.py`, `fake_data.py`, `notifications.py`,
  `print_schedule.py`, 6× `desktop/views/dialogs/`, `requests_page.py`).
  POZNAT preostao dug: 4 mjesta i dalje pozivaju `ZoneInfo("Europe/Sarajevo")`
  inline, bez `SARAJEVO` konstante — `appointments.py:336`,
  `availability.py:96,119`, `requests_panel.py:121` (REF-13 out-of-scope
  finding, budući REF-XX kandidat).
